...
# Cryptographically secure RNG for reward draws (avoids predictable rewards).
...
def _extract_token(request: Request) -> Optional[str]:
    """Get session token from cookie first, then Bearer header."""
...
def _session_expired(expires_at) -> bool:
    """Check if a stored expires_at value (datetime or ISO string) is in the past."""
...
async def get_current_user(request: Request) -> Optional[dict]:
    """Resolve the authenticated user (or None) from session token."""
    token = _extract_token(request)
    if not token:
        return None
    sess = await db.user_sessions.find_one({"session_token": token}, {"_id": 0})
    if not sess or _session_expired(sess.get("expires_at")):
        return None
...
async def _fetch_emergent_session(session_id: str) -> dict:
    """Call Emergent OAuth to exchange session_id for user data + session_token."""
...
async def _upsert_user(data: dict) -> str:
    """Create a new user (with welcome bonus) or update name/picture. Returns user_id."""
    ...
    await db.users.insert_one({
        ...
        "xp": 0, "coins": 500, "level": 1,
        "premium": False, "premium_until": None, "theme": "neon",
        "achievements": [], "high_scores": {},
        "last_daily_claim": None, "last_spin": None,
...
async def auth_session(response: Response, x_session_id: Optional[str] = Header(None)):
    ...
    user_id = await _upsert_user(data)
...
async def claim_daily(request: Request):
    ...
    reward = _rng.choice([50, 75, 100, 150])
    await db.users.update_one(
